  ...
  const applyFilter = () => {
    ...
    const applyFilterToCanvas = () => {
      if (canvas && context) {
        ...
        const offsetX = currentOrigin.current.x;
        const offsetY = currentOrigin.current.y;

        context.translate(
          translateX + offsetX * Math.cos(rotateAngle) + offsetY * Math.sin(rotateAngle),
          translateY + -offsetX * Math.sin(rotateAngle) + offsetY * Math.cos(rotateAngle)
        );
    ...
    if (imgElement.complete && imgElement.src) {
      ...
      imgRef.current.onload = applyFilterToCanvas;
    }
  };
